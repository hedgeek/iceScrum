...
class FinderController {
        ...
        @Secured('inProduct()')
        def list(String term, String tag) {
            withProduct { Product product ->
                def data = [:]

                term = term ? '%'+term+'%' : null
                data.actors =  searchInActors(product.id, [tag:tag, term:term, actor: params.withActors ? params.actor : null])
                data.stories = searchInStories(product.id, [tag:tag, term:term, story: params.withStories ? params.story : null])
                data.features = searchInFeatures(product.id, [tag:tag, term:term, feature: params.withFeatures ? params.feature : null])
                data.tasks = searchInTasks(product, [tag:tag, term:term, task: params.withTasks ? params.task : null])
